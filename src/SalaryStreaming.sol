...
contract SalaryStreaming {
    ...
    event disbursementSuccessful(address sender, address recipient, uint256 amount);
    ...
    function createStream(StreamDetails[] calldata _streamDetails, IntervalType intervalType) external {
        if (intervalType == IntervalType.Daily) {
            for (uint256 i = 0; i < _streamDetails.length; i++) {
                ...
                emit StreamCreated(_id, _streamDetails[i].recipient, IntervalType.Daily);
                idDailyCounter++;
            }
            ...
            for (uint256 i = 0; i < _streamDetails.length; i++) {
                ...
                emit StreamCreated(_id, _streamDetails[i].recipient, IntervalType.Monthly);
                idMonthlyCounter++;
            }
        }
    ...
    function disburseDaily() external {
        for (uint256 i = 0; i < dailyStreams.length; i++) {
            if (dailyStreams[i].active == true) {
                modalContract.transfer(dailyStreams[i].streamOwner, dailyStreams[i].recipient, dailyStreams[i].amount);

                emit disbursementSuccessful(
                    dailyStreams[i].streamOwner, dailyStreams[i].recipient, dailyStreams[i].amount
                );
            }
        }
    }
        ...
        for (uint256 i = 0; i < monthlyStreams.length; i++) {
            if (monthlyStreams[i].active == true) {
                modalContract.transfer(
                    monthlyStreams[i].streamOwner, monthlyStreams[i].recipient, monthlyStreams[i].amount
                );
